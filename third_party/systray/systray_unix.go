...
// SetIconName sets the systray icon by icon-theme name (the StatusNotifierItem
// IconName property). Hosts that support themed icons — KDE Plasma, XApp
// (Cinnamon), GNOME AppIndicator, XFCE — render the named theme icon as a crisp
// vector scaled to the panel, instead of resampling a fixed-size raster pixmap.
// Callers should still SetIcon a pixmap: the SNI spec lets both be set at once,
// and hosts that ignore IconName fall back to the pixmap, so no host is left
// without an icon.
...
// SetIconThemePath adds a directory the host should search for named icons (the
// StatusNotifierItem IconThemePath property). Use it to expose an app's own
// icons when they are not installed into a standard XDG icon theme (for example
// when running uninstalled from a build tree). No-op on hosts that ignore it.
func SetIconThemePath(dir string) {
	instance.lock.Lock()
	instance.iconThemePath = dir
	props := instance.props
	conn := instance.conn
	defer instance.lock.Unlock()
	...
	props.SetMust("org.kde.StatusNotifierItem", "IconThemePath", dir)
	if conn == nil {
		return
	}
	if err := notifier.Emit(conn, &notifier.StatusNotifierItem_NewIconThemePathSignal{
		Path: path,
		Body: &notifier.StatusNotifierItem_NewIconThemePathSignalBody{IconThemePath: dir},
	}); err != nil {
...
type tray struct {
	...
	iconData []byte
	// themed-icon state: IconName selects a vector icon from the icon theme
	// (crisp at any panel size), iconThemePath adds a search dir for it.
