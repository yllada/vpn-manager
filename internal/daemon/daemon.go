...
// CallDaemon calls a daemon method with the given params and result under the
// default daemon timeout. Falls back to fallbackFn if the daemon is not
// available. fallbackFn can be nil if there's no fallback (operation requires
// daemon).
func CallDaemon(method string, params, result any, fallbackFn func() error) error {
	...
	defer cancel()
	return CallDaemonWithContext(ctx, method, params, result, fallbackFn)
}

// CallDaemonWithContext is like CallDaemon but the caller supplies the context.
